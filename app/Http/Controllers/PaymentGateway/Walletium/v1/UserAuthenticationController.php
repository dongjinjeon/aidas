...
class UserAuthenticationController extends Controller
{
    ...
    protected $testuser_email       = "testuser@gmail.com";
    protected $testuser_password    = "testuser";
    ...
    public function showAuthForm($token) {
        ...
        if(request()->is("*/sandbox/*")) {
            ...
            $preview_url = route('walletium.pay.sandbox.v1.user.payment.preview',$token);
            $auth_form_submit = route('walletium.pay.sandbox.v1.user.auth.form.submit',$token);
        }else {
            // production request
            ...
            $preview_url = route('walletium.pay.v1.user.payment.preview',$token);
            $auth_form_submit = route('walletium.pay.v1.user.auth.form.submit',$token);
        }
        ...
        return view('walletium-gateway.pages.login',compact('page_title','payment_gateway_image','merchant_configuration','token','auth_form_submit'));
    }
    ...
    public function showMailVerify($token) {
        ...
        if(request()->is('*/sandbox/*')) {
            $form_submit_url = route('walletium.pay.sandbox.v1.user.auth.mail.verify.form.submit',$token);
        }else {
            $form_submit_url = route('walletium.pay.v1.user.auth.mail.verify.form.submit',$token);
        }
        ...
        return view('walletium-gateway.pages.mail-verify',compact('page_title','form_submit_url','token','payment_gateway_image'));
    }

    public function mailVerifySubmit(Request $request,$token) {
        $request->merge(['token' => $token]);
        $request->validate([
            'code'      => "required|string|exists:payment_order_requests,email_code",
            'token'     => "required|string|exists:payment_order_requests,token",
        ]);

        $request_record = PaymentOrderRequest::where('token',$token)->where('email_code',$request->code)->first();
        if(!$request_record) {
            $page_title = "Process Error";
            $data = [
                'title'         => "Payment request is invalid!",
        ...
        if(request()->is("*/sandbox/*")) {
            // sandbox request
            $preview_url = route('walletium.pay.sandbox.v1.user.payment.preview.submit',$token);
        }else {
            // production request
            $preview_url = route('walletium.pay.v1.user.payment.preview.submit',$token);
        } 
